...
class Character {
  ...
  isHome() {
    ...
    return this.position.x === this.type.roomAt && (
  ...
  compatriotBelow() {
    ...
    return this.position.neighbours.filter(n => n.x === position.x).filter(n => n.y === position.y + 1).some(n => n.char === this.type.char)
  }
  moves() {
    if (this.isHome()) {
      return [];
    ...
    var homeCells = this.homeLevels.map(y => key(this.type.roomAt, y)).map(k => this.position.cellMap[k]).filter(cell => cell.char === this.type.char || cell.isEmpty());
    ...
    var canStepTo: { cell: Cell, moves: number }[] = this.position.moveForward(this.position, 0);
    ...
    return canStepTo
      ...
      .filter(move => move.cell.y > 1 || (move.cell.y === 1 && !move.cell.isOutsideRoom()))
      .filter(move => move.cell.y === 1 || (move.cell.x === this.type.roomAt))
      .filter(move => move.cell.y === 1 || (move.cell.y > 1 && move.cell === home));
  }
...
class Cell {
  ...
  moveForward(from: Cell, moves: number): { cell: Cell, moves: number }[] {
    return this.steps(from).map(cell => {
      return [{ cell, moves: moves + 1 }, ...cell.moveForward(this, moves + 1)]
    }).flatMap(a => a);
  }
  steps(from: Cell): Cell[] {
    return this.neighbours.filter(n => n.isEmpty()).filter(n => from.x != n.x || from.y != n.y);
  }
}
...
class Game {
  ...
  constructor(
    ...
    var cells = state.map((line, y) => line.split('').map((char, x) => new Cell(x, y, char))).flatMap(a => a);
    var xRooms: number[] = state[2].split('').map((c, x) => ({ c, x })).filter(({ c }) => c !== '#').map(({ x }) => x);
    ...
    cells.filter(cell => !cell.isWall()).forEach(cell => {
      cell.neighbours = adjacent.map(adj => key(cell.x + adj.dx, cell.y + adj.dy)).map(k => cellMap[k]).filter(n => !!n && !n.isWall());
    });
    var costs = [1, 10, 100, 1000];
    var amphMap: { [key: string]: AmphipodType } = {};
    'ABCD'.split('').map((a, i) => ({ amph: a, cost: costs[i], roomAt: xRooms[i] })).forEach(a => { amphMap[a.amph] = new AmphipodType(a.amph, a.cost, a.roomAt) });
    this.amph = cells.filter(cell => cell.isCharacter()).map(cell => new Character(cell, amphMap[cell.char], this.homeLevels));
  }

  public getState() {
    var output: string[][] = this.state.map(line => line.split('').map(char => 'ABCD'.indexOf(char) !== -1 ? '.' : char));
    this.amph.forEach(a => {
  ...
  public move(x: number, y: number, toX: number, toY: number) {
    ...
    var moving = this.amph.find(character => character.position.x === x && character.position.y === y);
    if (!moving) {
...
class Puzzle {
  private games: { [key: string]: { game: string[], cost: number, resolved: boolean, previous: string[] | undefined } } = {};
  constructor(
    public input: Game
  ) {
    var state = input.getState();
    var k: string;
    this.games[keyFromState(state)] = { game: input.getState(), cost: 0, resolved: false, previous: undefined };
  }
  public solve() {
    var gamesSolved = 0;
    var minCost = Number.MAX_SAFE_INTEGER;
    var winningKey = '-...........-' + 'ABCD-'.repeat(this.input.homeLevels.length);
    var solved = false;
    var found = 0;
    while (!solved) {
      solved = true;
      const used = process.memoryUsage().heapUsed / 1024 / 1024;
      console.log(`${(elapsed() / 1000).toFixed(3).toString().padStart(10)}s ${(Math.round(used * 100) / 100).toFixed(2).padStart(8)} MB` +
        ` ${Object.values(this.games).length.toString().padStart(8)} games ${found.toString().padStart(7)} new` +
        ` solved: ${gamesSolved.toString().padStart(5)} minCost ${minCost.toString().padStart(8)}`);
      found = 0;
      for (var state of Object.values(this.games)) {
        if (!state.resolved) {
          ...
          var allMoves: Move[] = currentGame.amph.map(a => a.moves().map(m => ({ cell: m.cell, moves: m.moves, cost: m.moves * a.type.cost, character: a }))).flatMap(x => x);
          allMoves.forEach(move => {
            solved = false;
            solvedCurrent = false;
            var newGame = new Game(state.game, state.cost + move.cost);
            newGame.move(move.character.position.x, move.character.position.y, move.cell.x, move.cell.y);
            ...
            if (this.games[k]) {
              ...
              found++;
              this.games[k] = { game: newGameState, cost: newGame.cost, resolved: false, previous: state.game };
            }
            if (k === winningKey) {
              gamesSolved++;
    ...
    var primary = keys[0];
    // -...........-ABCD-ABCD-ABCD-ABCD-
    var moves: { game: string[], cost: number, resolved: boolean, previous: string[] | undefined }[] = [];
    ...
    for (var move of moves) {
      ...
      console.log(`${keyFromState(move.game)} + ${newCost.toFixed(0).padStart(4)} = ${move.cost.toFixed(0).padStart(6)}\n${move.game.join("\n")}\n`);
