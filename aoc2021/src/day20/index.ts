...
type Algorithm = {
  [key: string]: { key: string; char: string; i: number; bin: string };
};
...
const beside = adjacent
  .map((y) => adjacent.map((x) => ({ dx: x, dy: y })))
  .flatMap((a) => a);
console.log(beside);
...
const getAlgorithmKey = (
  ...
  var neighbours = beside
    .map((a) => ({ x: a.dx + x, y: a.dy + y }))
...
const toString = (image: Image) => {
  var maxX = Object.values(image)
    .map(({ x }) => x)
    .reduce((p, c) => (c > p ? c : p), Number.MIN_VALUE);
  var maxY = Object.values(image)
    .map(({ y }) => y)
    .reduce((p, c) => (c > p ? c : p), Number.MIN_VALUE);
  var minX = Object.values(image)
    .map(({ x }) => x)
    .reduce((p, c) => (c < p ? c : p), Number.MAX_VALUE);
  var minY = Object.values(image)
    .map(({ y }) => y)
    .reduce((p, c) => (c < p ? c : p), Number.MAX_VALUE);
  var result = "\n";
...
const evaluate = (algorithm: Algorithm, input: Image, even: boolean) => {
  ...
  Object.values(input).forEach((old) => {
    var { algorithmKey, neighbours } = getAlgorithmKey(
      old.x,
      old.y,
      input,
      inputIsNegative,
  ...
  Object.values(toProcess)
    ...
    .forEach((unp) => {
      var { algorithmKey } = getAlgorithmKey(
        unp.x,
        unp.y,
        input,
        inputIsNegative,
