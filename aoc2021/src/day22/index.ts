...
class Range {
  ...
  constructor(
    ...
    if (min > max) {
      console.log(`warn min > max! ${this.min} > ${this.max}`);
    }

    if (this.max === max && this.min === min && max >= min) {
      return;
    }
    log(`warn ${this.min} => ${min}, ${this.max} => ${max}`);
...
class Grid {
  ...
  constructor(
    ...
    for (var x = -50; x <= 50; x++) {
      ...
      for (var y = -50; y <= 50; y++) {
        var iy = y + 50;
        this.grid[ix][iy] = new Array(101);
        for (var z = -50; z <= 50; z++) {
          var iz = z + 50;
          this.grid[ix][iy][iz] = 0;
        }
      }
    }
  }
  set(value: number, x: number, y: number, z: number) {
    if (this.inRange(x) && this.inRange(y) && this.inRange(z)) {
      if (this.grid[x + 50][y + 50][z + 50] === value) {
        return;
      }
      if (value === 1) {
        this.on++;
      ...
      this.grid[x + 50][y + 50][z + 50] = value;
    }
  }
  ...
  sum() {
    // return [this.on, this.grid.reduce((xp, xc) => xp + xc.reduce((yp, yc) => yp + yc.reduce((zp, zc) => zp + zc, 0), 0), 0)];
    return this.on;
  }
}
class Volume {
  ...
  intersection(that: Volume): Volume | null {
    var xCommon = this.xRange.overlap(that.xRange);
    if (xCommon) {
      var yCommon = this.yRange.overlap(that.yRange);
      if (yCommon) {
        var zCommon = this.zRange.overlap(that.zRange);
        if (zCommon) {
          // if both set - return 0 in order to not double count the region
          // else use the new value (in that)
          // if neither set return 1 in order to not double delete
          // there's probably a neat boolean operation to return the right result here ... but for clarity
          ...
          if (this.set & that.set) {
            set = 0;
          } else if (this.set | that.set) {
            set = that.set;
          } else {
            set = 1;
          }
          return new Volume(set, xCommon, yCommon, zCommon);
        }
      }
    }
...
class Puzzle {
  ...
  public solve() {
    ...
    for (var rule of this.rules) {
      for (var x = rule.xRange.min; x <= rule.xRange.max; x++) {
        for (var y = rule.yRange.min; y <= rule.yRange.max; y++) {
          for (var z = rule.zRange.min; z <= rule.zRange.max; z++) {
            this.grid.set(rule.set, x, y, z);
          }
        }
      }
    }
  ...
  public solve2() {
    ...
    for (var rule of this.rules) {
      ...
      if (rule.set) {
        // on
        // find any intersections and add them (they will be positive where we are intersecting a -ve and vice versa)
        // add the volume itself
        this.volumes = [...this.volumes, ...intersections, volume];
      } else {
        // off
        // just add the intersections (which will all be -ve)

        this.volumes = [...this.volumes, ...intersections];
      }
      i++;
      // console.log(`${i} : ${rule.set} : ${intersections.length} : ${this.volumes.length}`)
    }
    return this.volumes.reduce((p, c) => p + c.size, 0);
  }
}
