...
const parseInput = (rawInput: string) => {
  return rawInput
    ...
    .map((line) => {
      ...
      return parts.map(getBinaryResult);
    });
};
...
const getMask = (utf16: number): [number, boolean] => {
  if (utf16 > 90) {
...
const getBinaryResult = (line: string) => {
  let upperBits: number = 0;
  let lowerBits: number = 0;

  for (let i = 0; i < line.length; i++) {
    const [mask, upper] = getMask(line.charCodeAt(i));
    if (upper) {
...
const getPriority = (lower: number, upper: number): number => {
  return ((lower === 0) ? 26 : 0) + Math.log2(lower | upper);
};
...
const part1 = (rawInput: string) => {
  ...
  return input
    .map((candidates) => {
      ...
      return getPriority(lower, upper);
    })
    .reduce((p, c) => p + c, 0);
};
